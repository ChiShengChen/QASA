...
class Task:
    ...
    def generate_data(self, total_points=500, seq_len_train=20, train_split=0.8, period=10, seed=42):
        np.random.seed(seed)
        t = np.linspace(0, 50, total_points)
        waveform = 2 * (t / period - np.floor(0.5 + t / period))
...
def generate_sawtooth_wave(t, period=10):
    # t: numpy array
    return 2 * (t / period - np.floor(0.5 + t / period))

def run_classical_sawtooth_wave_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical sawtooth wave prediction test")
    seq_len_train = 20
    n_embd_test = 16
    n_head_test = 2
    n_layer_test = 1
    ...
    print(f"Classical Model Config for Sawtooth Wave: {config}\n")
    try:
        ...
        raise
    print("Classical RWKVModel for sawtooth wave instantiated successfully.\n")
    total_points = 500
    t = np.linspace(0, 50, total_points)
    period = 10
    waveform = generate_sawtooth_wave(t, period).astype(np.float32)
    ...
    all_epoch_losses = []
    model.train()
    print("Starting classical training for sawtooth wave prediction...")
    training_start_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = "results_sawtooth_wave_classical"
    ...
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Sawtooth Wave', color='blue', linestyle='-')
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Sawtooth Wave (Classical)', color='green', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Sawtooth Wave (Classical RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"sawtooth_wave_comparison_classical_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting classical generation for sawtooth wave prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    print(f"Generated sawtooth wave (first 20 points): {generated_waveform_for_eval[:20].tolist()}")
    print(f"True sawtooth wave (first 20 points):      {true_waveform_part_for_eval[:20].tolist()}")
    print(f"Mean Absolute Error (MAE, classical): {mae:.6f}")
    print(f"Mean Squared Error (MSE, classical):  {mse:.6f}\n")
    plot_filename = os.path.join(results_dir, f"sawtooth_wave_comparison_classical_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Sawtooth Wave', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Sawtooth Wave (Classical)', color='green', linestyle='--')
    plt.title('Ground Truth vs. Predicted Sawtooth Wave (Classical RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"c_sawtooth_wave_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Classical', 'SawtoothWave',
    ...
    print("\n=== Finished Classical Sawtooth Wave Prediction Test ===\n")

if __name__ == '__main__':
    run_classical_sawtooth_wave_prediction() 