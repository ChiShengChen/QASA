...
class Task:
    ...
    def generate_data(self, total_points=500, seq_len_train=20, train_split=0.8, ar_params=[0.75, -0.25], ma_params=[0.65], seed=42):
        np.random.seed(seed)
...
def run_quantum_arma_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum ARMA prediction test")
    seq_len_train = 20
    n_embd_test = 16
    n_head_test = 2
    n_layer_test = 1
    ...
    print(f"Quantum Model Config for ARMA: {config}\n")
    try:
        ...
        raise
    print("Quantum RWKVModel for ARMA instantiated successfully.\n")
    total_points = 500
    ar_params = [0.75, -0.25]
    ma_params = [0.65]
    ...
    results_dir = "results_arma_quantum"
    os.makedirs(results_dir, exist_ok=True)
    print("Starting quantum training for ARMA prediction...")
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted ARMA (Quantum)', color='red', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted ARMA (Quantum RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"arma_comparison_quantum_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting quantum generation for ARMA prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    print(f"Generated ARMA (Quantum, first 20 points): {generated_waveform_for_eval[:20].tolist()}")
    ...
    results_dir = "results_arma_quantum"
    os.makedirs(results_dir, exist_ok=True)
    plot_filename = os.path.join(results_dir, f"arma_comparison_quantum_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth ARMA', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted ARMA (Quantum)', color='red', linestyle='--')
    plt.title('Ground Truth vs. Predicted ARMA (Quantum RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"q_arma_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Quantum', 'ARMA',
    ...
    print("\n=== Finished Quantum ARMA Prediction Test ===\n")

if __name__ == '__main__':
    run_quantum_arma_prediction() 