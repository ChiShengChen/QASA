...
class Task:
    ...
    def generate_data(self, total_points=500, seq_len_train=20, train_split=0.8, seed=42):
        np.random.seed(seed)
        ...
        regimes = [
            ...
            (200, 300, 0.0, 0, 0.01),
            (300, 350, 0.2, -40, 0.02),
            (350, 400, -0.15, 40, 0.02),
            (400, 500, 0.03, -10, 0.01)
        ]
...
def run_quantum_piecewise_regime_prediction():
    ...
    print(f"Quantum Model Config for Piecewise/Regime: {config}\n")
    try:
        model = QuantumRWKVModel(config)
    except Exception as e:
        print(f"Error instantiating QuantumRWKVModel for piecewise/regime: {e}")
        raise
    print("QuantumRWKVModel for piecewise/regime instantiated successfully.\n")
    total_points = 500
    ...
    results_dir = "results_piecewise_regime_quantum"
    os.makedirs(results_dir, exist_ok=True)
    print("Starting quantum training for piecewise/regime prediction...")
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Piecewise Regime (Quantum)', color='red', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Piecewise Regime (Quantum RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"piecewise_regime_comparison_quantum_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting quantum generation for piecewise/regime prediction...")
    generated_waveform_points = []
    ...
    results_dir = "results_piecewise_regime_quantum"
    os.makedirs(results_dir, exist_ok=True)
    plot_filename = os.path.join(results_dir, f"piecewise_regime_comparison_quantum_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Piecewise/Regime', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Piecewise/Regime (Quantum)', color='red', linestyle='--')
    plt.title('Ground Truth vs. Predicted Piecewise/Regime (Quantum RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"q_piecewise_regime_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Quantum', 'PiecewiseRegime',
...
if __name__ == '__main__':
    run_quantum_piecewise_regime_prediction() 