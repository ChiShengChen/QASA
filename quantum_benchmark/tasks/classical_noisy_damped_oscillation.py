...
def run_classical_noisy_damped_oscillation_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical noisy damped oscillation prediction test")
    seq_len_train = 20
    n_embd_test = 16
    n_head_test = 2
    n_layer_test = 1
    ...
    print(f"Classical Model Config for Noisy Damped Oscillation: {config}\n")
    try:
        ...
        raise
    print("Classical RWKVModel for noisy damped oscillation instantiated successfully.\n")
    total_points = 500
    ...
    all_epoch_losses = []
    model.train()
    print("Starting classical training for noisy damped oscillation prediction...")
    training_start_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = "results_noisy_damped_classical"
    ...
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted (Classical)', color='green', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Noisy Damped Oscillation (Classical RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"noisy_damped_comparison_classical_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting classical generation for noisy damped oscillation prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    print(f"Generated Noisy Damped Oscillation (classical, first 20 points): {generated_waveform_for_eval[:20].tolist()}")
    ...
    plot_filename = os.path.join(results_dir, f"noisy_damped_comparison_classical_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Noisy Damped Oscillation', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted (Classical)', color='green', linestyle='--')
    plt.title('Ground Truth vs. Predicted Noisy Damped Oscillation (Classical RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"c_noisy_damped_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Classical', 'NoisyDampedOsc',
    ...
    print("\n=== Finished Classical Noisy Damped Oscillation Prediction Test ===\n")

if __name__ == '__main__':
    run_classical_noisy_damped_oscillation_prediction() 