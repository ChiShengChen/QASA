...
def run_quantum_sawtooth_wave_prediction():
    ...
    print(f"Quantum Model Config for Sawtooth Wave: {config}\n")
    try:
        model = QuantumRWKVModel(config)
    except Exception as e:
        print(f"Error instantiating QuantumRWKVModel for sawtooth wave: {e}")
        raise
    print("QuantumRWKVModel for sawtooth wave instantiated successfully.\n")
    total_points = 500
    t = np.linspace(0, 50, total_points)
    period = 10
    ...
    results_dir = "results_sawtooth_wave_quantum"
    os.makedirs(results_dir, exist_ok=True)
    print("Starting quantum training for sawtooth wave prediction...")
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Sawtooth Wave (Quantum)', color='red', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Sawtooth Wave (Quantum RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"sawtooth_wave_comparison_quantum_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting quantum generation for sawtooth wave prediction...")
    generated_waveform_points = []
    ...
    results_dir = "results_sawtooth_wave_quantum"
    os.makedirs(results_dir, exist_ok=True)
    plot_filename = os.path.join(results_dir, f"sawtooth_wave_comparison_quantum_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Sawtooth Wave', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Sawtooth Wave (Quantum)', color='red', linestyle='--')
    plt.title('Ground Truth vs. Predicted Sawtooth Wave (Quantum RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"q_sawtooth_wave_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Quantum', 'SawtoothWave',
...
if __name__ == '__main__':
    run_quantum_sawtooth_wave_prediction() 