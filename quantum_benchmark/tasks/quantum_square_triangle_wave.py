...
class Task:
    ...
    def generate_data(self, total_points=500, seq_len_train=20, train_split=0.8, seed=42, wave_type="square"):
        np.random.seed(seed)
        t = np.linspace(0, 50, total_points)
        period = 5 + 5 * np.sin(0.1 * t)
        duty = 0.2 + 0.6 * (0.5 + 0.5 * np.sin(0.05 * t))
        if wave_type == "square":
            waveform = np.array([1.0 if ((tt % pp) / pp) < dd else -1.0 for tt, pp, dd in zip(t, period, duty)], dtype=np.float32)
        elif wave_type == "triangle":
            waveform = np.array([4 * np.abs(((tt % pp) / pp) - 0.5) - 1 for tt, pp in zip(t, period)], dtype=np.float32)
        else:
            raise ValueError("wave_type must be 'square' or 'triangle'")
...
def run_quantum_square_triangle_wave_prediction():
    ...
    print(f"Quantum Model Config for Square/Triangle Wave: {config}\n")
    try:
        model = QuantumRWKVModel(config)
    except Exception as e:
        print(f"Error instantiating QuantumRWKVModel for square/triangle wave: {e}")
        raise
    print("QuantumRWKVModel for square/triangle wave instantiated successfully.\n")
    total_points = 500
    t = np.linspace(0, 50, total_points)
    period = 5 + 5 * np.sin(0.1 * t)
    duty = 0.2 + 0.6 * (0.5 + 0.5 * np.sin(0.05 * t))
    ...
    for wave_type, waveform in zip(["square", "triangle"], [square_wave, triangle_wave]):
        ...
        results_dir = f"results_square_triangle_quantum/{wave_type}"
        ...
        all_epoch_losses = []
        model.train()
        print(f"Starting quantum training for {wave_type} wave prediction...")
        for epoch in range(num_epochs):
            ...
            if num_windows_processed > 0:
                ...
                if (epoch + 1) % 100 == 0:
                    ...
                    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label=f'Ground Truth {wave_type.capitalize()} Wave', color='blue', linestyle='-')
                    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label=f'Predicted {wave_type.capitalize()} Wave (Quantum)', color='red', linestyle='--')
                    plt.title(f'Ground Truth vs. Predicted {wave_type.capitalize()} Wave (Quantum RWKV) - Epoch {epoch+1}')
                    plt.xlabel('Time Step (in test segment)')
                    plt.ylabel('Value')
                    plt.legend()
                    plt.grid(True)
                    plt.tight_layout()
                    plot_filename = os.path.join(results_dir, f"{wave_type}_wave_comparison_quantum_{training_start_time}_epoch{epoch+1}.png")
                    try:
        ...
        model.eval()
        print(f"Starting quantum generation for {wave_type} wave prediction...")
        generated_waveform_points = []
        ...
        plot_filename = os.path.join(results_dir, f"{wave_type}_wave_comparison_quantum_{training_start_time}_final.png")
        plt.figure(figsize=(14, 7))
        ...
        plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label=f'Predicted {wave_type.title()} Wave (Quantum)', color='red', linestyle='--')
        plt.title(f'Ground Truth vs. Predicted {wave_type.title()} Wave (Quantum RWKV)')
        plt.xlabel('Time Step (in test segment)')
        ...
        experiment_id = f"q_{wave_type}_{timestamp}"
        learning_rate = optimizer.param_groups[0]['lr']
        data_row = [
            timestamp, experiment_id, 'Quantum', wave_type.title(),
        ...
        print(f"\n=== Finished Quantum {wave_type.title()} Wave Prediction Test ===\n")

if __name__ == '__main__':
    run_quantum_square_triangle_wave_prediction() 