...
class Task:
    ...
    def generate_data(self, total_points=500, ar_params=[0.75, -0.25], ma_params=[0.65], seed=42):
        """
        產生訓練/測試資料
        Args:
            total_points: 總數據點數
            ...
            seed: 隨機種子
        """
        waveform = generate_arma_sequence(total_points, ar_params, ma_params, seed)
...
def generate_arma_sequence(total_points=500, ar_params=[0.75, -0.25], ma_params=[0.65], seed=42):
    np.random.seed(seed)
    ar = np.r_[1, -np.array(ar_params)]  # add 1 for zero-lag
    ma = np.r_[1, np.array(ma_params)]
    arma_process = ArmaProcess(ar, ma)
    y = arma_process.generate_sample(nsample=total_points)
    return y.astype(np.float32)

def run_classical_arma_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical ARMA prediction test")
    seq_len_train = 20
    n_embd_test = 16
    n_head_test = 2
    n_layer_test = 1
    ...
    print(f"Classical Model Config for ARMA: {config}\n")
    try:
        ...
        raise
    print("Classical RWKVModel for ARMA instantiated successfully.\n")
    total_points = 500
    ar_params = [0.75, -0.25]  # AR(2)
    ma_params = [0.65]         # MA(1)
    waveform = generate_arma_sequence(total_points, ar_params, ma_params)
    ...
    all_epoch_losses = []
    model.train()
    print("Starting classical training for ARMA prediction...")
    training_start_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = "results_arma_classical"
    ...
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth ARMA', color='blue', linestyle='-')
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted ARMA (Classical)', color='green', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted ARMA (Classical RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"arma_comparison_classical_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting classical generation for ARMA prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    print(f"Generated ARMA (first 20 points): {generated_waveform_for_eval[:20].tolist()}")
    print(f"True ARMA (first 20 points):      {true_waveform_part_for_eval[:20].tolist()}")
    print(f"Mean Absolute Error (MAE, classical): {mae:.6f}")
    print(f"Mean Squared Error (MSE, classical):  {mse:.6f}\n")
    plot_filename = os.path.join(results_dir, f"arma_comparison_classical_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth ARMA', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted ARMA (Classical)', color='green', linestyle='--')
    plt.title('Ground Truth vs. Predicted ARMA (Classical RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"c_arma_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Classical', 'ARMA',
