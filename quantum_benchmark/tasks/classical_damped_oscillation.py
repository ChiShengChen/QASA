...
def run_classical_damped_oscillation_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for classical damped oscillation prediction test")
    ...
    print(f"Classical Model Config for Damped Oscillation: {config}\n")
    ...
    try:
        ...
        raise
    print("Classical RWKVModel for damped oscillation instantiated successfully.\n")
    ...
    model.train()
    print("Starting classical training for damped oscillation prediction...")
    training_start_time = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    results_dir = "results_damped_oscillation_classical"
    ...
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Damped Oscillation (Classical)', color='green', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Damped Oscillation (Classical RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Oscillation Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"damped_oscillation_comparison_classical_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting classical generation for damped oscillation prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    results_dir = "results_damped_oscillation_classical"
    os.makedirs(results_dir, exist_ok=True)
    plot_filename = os.path.join(results_dir, f"damped_oscillation_comparison_classical_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Damped Oscillation', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Damped Oscillation (Classical)', color='green', linestyle='--')
    plt.title('Ground Truth vs. Predicted Damped Oscillation (Classical RWKV)')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Oscillation Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"c_damped_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Classical', 'DampedOscillation',
...
if __name__ == '__main__':
    run_classical_damped_oscillation_prediction() 