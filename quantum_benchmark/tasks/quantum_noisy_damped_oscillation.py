...
class Task:
    ...
    def generate_data(self, total_points=500, seq_len_train=20, train_split=0.8, amplitude=1.0, damping=0.05, freq=0.2, noise_std=0.05, seed=42):
        np.random.seed(seed)
        t = np.arange(total_points)
        clean = amplitude * np.exp(-damping * t) * np.sin(2 * np.pi * freq * t)
        ...
        train_split_idx = int(total_points * train_split)
...
def generate_noisy_damped_oscillation(total_points=500, amplitude=1.0, damping=0.05, freq=0.2, noise_std=0.05, seed=42):
    np.random.seed(seed)
    t = np.arange(total_points)
    clean = amplitude * np.exp(-damping * t) * np.sin(2 * np.pi * freq * t)
    noise = np.random.normal(0, noise_std, size=clean.shape)
...
def run_quantum_noisy_damped_oscillation_prediction():
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"Using device: {device} for quantum noisy damped oscillation prediction test")
    seq_len_train = 20
    n_embd_test = 16
    n_head_test = 2
    n_layer_test = 1
    ...
    print(f"Quantum Model Config for Noisy Damped Oscillation: {config}\n")
    try:
        ...
        raise
    print("Quantum RWKVModel for noisy damped oscillation instantiated successfully.\n")
    total_points = 500
    waveform = generate_noisy_damped_oscillation(total_points)
    ...
    results_dir = "results_noisy_damped_quantum"
    os.makedirs(results_dir, exist_ok=True)
    print("Starting quantum training for noisy damped oscillation prediction...")
    for epoch in range(num_epochs):
        ...
        if num_windows_processed > 0:
            ...
            if (epoch + 1) % 100 == 0:
                ...
                plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Noisy Damped Oscillation', color='blue', linestyle='-')
                plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Noisy Damped Oscillation (Quantum)', color='red', linestyle='--')
                plt.title(f'Ground Truth vs. Predicted Noisy Damped Oscillation (Quantum RWKV) - Epoch {epoch+1}')
                plt.xlabel('Time Step (in test segment)')
                plt.ylabel('Value')
                plt.legend()
                plt.grid(True)
                plt.tight_layout()
                plot_filename = os.path.join(results_dir, f"noisy_damped_oscillation_comparison_quantum_{training_start_time}_epoch{epoch+1}.png")
                try:
    ...
    model.eval()
    print("Starting quantum generation for noisy damped oscillation prediction...")
    generated_waveform_points = []
    current_input_sequence = X_test_seed.clone().to(device)
    num_points_to_generate = Y_test_true_full.shape[1]
    param_dtype = next(model.parameters()).dtype
    ...
    print(f"Generated Noisy Damped Oscillation (quantum, first 20 points): {generated_waveform_for_eval[:20].tolist()}")
    print(f"True Noisy Damped Oscillation (first 20 points):      {true_waveform_part_for_eval[:20].tolist()}")
    print(f"Mean Absolute Error (MAE, quantum): {mae:.6f}")
    print(f"Mean Squared Error (MSE, quantum):  {mse:.6f}\n")
    results_dir = "results_noisy_damped_quantum"
    os.makedirs(results_dir, exist_ok=True)
    plot_filename = os.path.join(results_dir, f"noisy_damped_oscillation_comparison_quantum_{training_start_time}_final.png")
    plt.figure(figsize=(14, 7))
    plt.plot(np.arange(len(true_waveform_part_for_eval)), true_waveform_part_for_eval, label='Ground Truth Noisy Damped Oscillation', color='blue', linestyle='-')
    plt.plot(np.arange(len(generated_waveform_for_eval)), generated_waveform_for_eval, label='Predicted Noisy Damped Oscillation (Quantum)', color='red', linestyle='--')
    plt.title(f'Ground Truth vs. Predicted Noisy Damped Oscillation (Quantum RWKV) - Final')
    plt.xlabel('Time Step (in test segment)')
    plt.ylabel('Value')
    plt.legend()
    plt.grid(True)
    ...
    experiment_id = f"q_noisy_damped_{timestamp}"
    learning_rate = optimizer.param_groups[0]['lr']
    data_row = [
        timestamp, experiment_id, 'Quantum', 'NoisyDampedOsc',
    ...
    print("\n=== Finished Quantum Noisy Damped Oscillation Prediction Test ===\n")

if __name__ == '__main__':
    run_quantum_noisy_damped_oscillation_prediction() 